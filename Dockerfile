...
# Install backend dependencies first (clean)
COPY server/package*.json ./server/
RUN cd server && npm install --production && npm rebuild sqlite3

# Copy source code (will skip node_modules because of .dockerignore)
COPY server/ ./server/
